---
title: "Clustering of US States based on COVID Statistics"
...
**1. Hierarchical methods:** Two types of hierarchical methods exist:  
    a. The agglomerative hierarchical method is a "bottom-up" approach which starts by defining each observations a cluster. Then, the two closest clusters, measured using some distance measure are combined into a new cluster. In each subsequent step, two existing clusters are merged into a single cluster.  
    b. The divisive hierarchical method is a "top-down" approach in which all observations start in one cluster. Then, the initial cluster is divided into two clusters. In each subsequent step, the existing cluster is divided into two clusters.
...
This study uses the most popular method called K-means clustering that falls under non-hierarchical method to cluster US states based on three important COVID statistics measures. I plan to demonstrate the agglomerative hierarchical clustering and model based clustering in future posts.
...
Let $\overline{x}_{sk}$ be the mean of the k^th^ cluster for the x~s~ variable, s = 1,2,..p. The within-cluster sum of squares for the kth cluster is given by;
...
The plot shows that the average silhouttes is maximized for 2 clusters followed by 3 and 4. We will see what the third method suggest.
...
Where En* denotes expectation under a sample of size n from the reference distribution. Wk is the pooled witin cluster sum of squares around the cluster means. The optimal value of k will be the one maximizing Gapn(k).
